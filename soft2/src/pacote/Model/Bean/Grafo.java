...
public class Grafo {
    ...
    public Grafo(List<Almacen> listAlmacen) {
        ...
        for (int v = 0; v < V; v++) {
        	...
        	System.out.println(almacen.id);
        	
        	map.put(almacen.id, v);
    ...
    public void rutas(){
    	...
    	for(int i=0; i<V; i++){
    		System.out.println("Desde la ciudad " + Almacenes.get(i).id+ " salen los sgtes vuelos: ");
    		for(int j=0; j<adj[i].size(); j++){
    			Vuelo vuelo = adj[i].get(j);
    			System.out.println("== " + vuelo.id + ") --> " + vuelo.ciudad_fin + " , hr ini : " + vuelo.hora_inicio + " , hr_fin : " + vuelo.hora_fin);
    		}
    		System.out.println();
    	}
    }
